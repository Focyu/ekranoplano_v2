...
// C/C++ source code generated on : Sat Apr 25 18:31:53 2026
//
#ifdef _MSC_VER
#pragma warning(push)
...
void NodeInterface::initialize(int argc, char * const argv[]) {
    try {
        ...
        std::string NodeName("pid_control_V1_OLD");
        SLROSNodePtr = std::make_shared<rclcpp::Node>(NodeName);
        RCLCPP_INFO(SLROSNodePtr->get_logger(),"** Starting the model \"pid_control_V1_OLD\" **\n");
        mExec = std::make_shared<rclcpp::executors::SLMultiThreadedExecutor>();
        mExec->add_node(SLROSNodePtr);
        //initialize the model which will initialize the publishers and subscribers
        mModel = std::make_shared<pid_control_V1_OLD>(
        );
